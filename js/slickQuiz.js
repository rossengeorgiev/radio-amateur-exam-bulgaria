(function($){
    ...
    $.slickQuiz = function(element, options) {
        ...
        var triggers = {
			...
			checknext:       '#' + selector + ' .checkNextQuestion'
        }
        ...
        plugin.method = {
            ...
            setupQuiz: function() {
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        questionHTML.append('<div class="questionCount">Въпрос <span class="current">' + count + '</span> от <span class="total">' + questionCount + '</span></div>');
                        ...
                        if (plugin.config.disableResponseMessaging || plugin.config.completionResponseMessaging) {
                            questionHTML.append('<a href="" class="button checkNextQuestion">' + plugin.config.nextQuestionText + '</a>');
                        } else {
            ...
            checkAnswer: function(checkButton) {
                ...
                if (plugin.config.preventUnanswered && selectedAnswers.length == 0) {
                    alert('Изберете поне един отговор!');
                    return false;
                }
            ...
            calculateLevel: function(correctAnswers) {
                ...
                if (plugin.method.inRange(0, 0.26, percent)) {
                    level = 5;
                } else if (plugin.method.inRange(0.27, 0.53, percent)) {
                    level = 4;
                } else if (plugin.method.inRange(0.54, 0.79, percent)) {
                    level = 3;
                } else if (plugin.method.inRange(0.80, 0.99, percent)) {
                    level = 2;
                } else {
                    level = 1;
                }
        ...
        plugin.init = function() {
            ...
            $(triggers.starter).bind('click', function(e) {
                e.preventDefault();
                return plugin.method.startQuiz(this);
            });
            ...
            $(triggers.checker).live('click', function(e) {
                e.preventDefault();
                return plugin.method.checkAnswer(this);
            });

            // Bind "back" button
            $(triggers.back).live('click', function(e) {
                e.preventDefault();
                return plugin.method.backToQuestion(this);
            });

            // Bind "next question" button
            $(triggers.next).live('click', function(e) {
                e.preventDefault();
                return plugin.method.nextQuestion(this);
            });
            // Bind "checknext question" button
            $(triggers.checknext).live('click', function(e) {
                e.preventDefault();
                if(plugin.method.checkAnswer(this)) plugin.method.nextQuestion(this);
            });
		}
